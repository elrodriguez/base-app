...
const form = useForm({
    name: props.resmenu.name,
    description: props.resmenu.description,
    status: props.resmenu.status == 1 ? true : false
});

const updateMenu = () => {
    form.put(route('res_menus_update',props.resmenu.id), {
        errorBag: 'updateMenu',
        preserveScroll: true,
        onSuccess: () => {
            Swal2.fire({
...
<template>
    <FormSection @submitted="updateMenu" class="">
        <template #title>
            Menu Detalles
        </template>

        <template #description>
            Crear editar Menu, Los campos con * son obligatorios
        </template>
