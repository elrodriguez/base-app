...
onMounted(() => {
    ...
    presentations.value = props.presentations;
});
...
const form = useForm({
    id: props.comanda.id,
    name: props.comanda.name,
    image: null,
    image_preview: assetUrl + 'storage/' + props.comanda.image,
    category_id: props.comanda.category_id,
    presentation_id: props.comanda.presentation_id,
    description: props.comanda.description,
    price: props.comanda.price,
    status: props.comanda.status == 1 ? true : false
});

const updateComanda = () => {
    form.post(route('res_comandas_update'), {
        forceFormData: true,
        errorBag: 'updateComanda',
        preserveScroll: true,
        onSuccess: () => {
            Swal2.fire({
                title: 'Enhorabuena',
...
<template>
    <FormSection @submitted="updateComanda" class="">
        <template #title>
            Comanda Detalles
        </template>

        <template #description>
            Crear editar Comanda, Los campos con * son obligatorios
        </template>
