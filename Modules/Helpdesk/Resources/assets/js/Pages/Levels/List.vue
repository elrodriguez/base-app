<script setup>
    ...
    const selectAllCheckboxMembers = (event) => {
        if(event.target.checked){
            const allUsers = props.users;
            for (let i = 0; i < allUsers.length; i++) {
                formMembers.user_ids[i] = allUsers[i].id;
            }
        }else{
            formMembers.user_ids = [];
        }
                    ...
                    <div class="max-w-full overflow-x-auto">
                        ...
                        <thead class="border-b border-stroke">
                            <tr class="bg-gray-50 text-left dark:bg-meta-4">
                                <th class="w-4 py-4 px-4 font-medium text-black dark:text-white xl:pl-11">#</th>
                                <th class="w-8 py-4 px-4 text-center  font-medium text-black dark:text-white">
                                    Acción
                                </th>
    ...
    <ModalSmall :show="displayModalMembers" :onClose="closeModalMembers" :icon="'/img/seguidores.png'">
        ...
        <template #content>
            <table class="w-full table-auto">
                <thead class="border-b border-stroke">
                    <tr class="bg-gray-50 text-left dark:bg-meta-4">
                        <th class="py-2 text-center font-medium text-black dark:text-white">
                            <input @change="selectAllCheckboxMembers($event)" id="checkboxAll" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        </th>
                        <th class="py-2 font-medium text-black dark:text-white">Nombre de Usuario</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(user, index) in users" class="bg-white border-b border-stroke dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="w-4 p-4">
                            <div class="flex items-center">
                                <input v-model="formMembers.user_ids" :value="user.id" :id="'checkbox-table-'+index" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-stroke rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                <label :for="'checkbox-table-'+index" class="sr-only">checkbox</label>
                            </div>
                        </td>
                        <td class="bg-white border-b border-stroke dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            {{ user.name  }}
                        </td>
                    </tr>
                </tbody>
