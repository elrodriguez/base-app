...
if (form.ubigeo) {
    const ubigeo = props.ubigeo.find(
        (row) => row.district_id.toLowerCase() === form.ubigeo.toLowerCase()
    );
    form.ubigeo_description = ubigeo
...
const filterCities = () => {
    ...
    searchUbigeos.value = props.ubigeo.filter((row) =>
        row.district_name
            .toLowerCase()
            .includes(form.ubigeo_description.toLowerCase())
    );
};
const selectCity = (item) => {
    form.ubigeo_description =
...
<template>
    <FormSection @submitted="createOrUpdateCompany">
        ...
        <template #description> Detalles </template>
        ...
        <template #form>
            ...
            <div class="col-span-6 sm:col-span-3">
                ...
                />
                <InputError
                    :message="form.errors.fiscal_address"
                    class="mt-2"
                />
            </div>
            <div class="col-span-6 sm:col-span-3">
            ...
            <div class="col-span-6">
                ...
                <div class="relative">
                    ...
                    />
                    <ul
                        v-if="searchUbigeos && searchUbigeos.length > 0"
                        class="list-disc list-inside absolute z-50 w-full bg-white border border-gray-300 rounded-md mt-1"
                    >
                        <li
                            v-for="item in searchUbigeos"
                            :key="item.id"
                            class="px-4 cursor-pointer hover:bg-gray-100"
            ...
            <div class="col-span-6 sm:col-span-3">
                <figure v-if="form.logo_document_old" class="max-w-lg">
                    <img
                        class="h-auto max-w-full rounded-lg"
                        :src="'/storage/' + form.logo_document_old"
                        alt="image description"
                    />
                    <figcaption
                        class="mt-2 text-sm text-center text-gray-500 dark:text-gray-400"
                ...
                <label
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                    for="small_size"
                    >Logo - se recomienda 317x48</label
                >
                <input
                    type="file"
                    @input="form.logo_document = $event.target.files[0]"
                    class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                    id="document_small_size"
                />
            </div>
            ...
            <div class="col-span-6 sm:col-span-2">
                <label
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                    for="small_size"
                    >Certificado Sunat</label
                >
                <input
                    @input="form.certificate_sunat = $event.target.files[0]"
                    class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                    id="small_size"
                    type="file"
                />
